...
export default class AuthService {
    ...
    private readonly props: AuthServiceProps = {
        clientId: "bbp-braynscircuitstudio",
        redirectUri: window.location.origin,
        ssoUrl: "https://bbpauth.epfl.ch/auth/realms/BBP/protocol/openid-connect/auth",
        tokenUrl:
    ...
    public clearKeyCloackCookie() {
        const keysToRemove: string[] = []
        for (let index = 0; index < window.localStorage.length; index++) {
            const key = window.localStorage.key(index)
            if (!key) continue
            if (key.indexOf("bbpauth.epfl.ch") !== NOT_FOUND) {
    ...
    private async generateCodeVerifier() {
        ...
        const cachedCodeVerifierExpiresAt = await this.storage.load(
