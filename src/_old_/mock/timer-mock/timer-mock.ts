...
const overridenWindowObject = window as unknown as {
    setTimeout(this: void, slot: () => void, delay?: number): number
    clearTimeout(this: void, id: number): void
    setInterval(this: void, slot: () => void, delay?: number): number
    clearInterval(this: void, id: number): void
}
...
class TimerMock {
    ...
    private readonly _setTimeout = overridenWindowObject.setTimeout

    private readonly _clearTimeout = overridenWindowObject.clearTimeout

    private readonly _setInterval = overridenWindowObject.setInterval

    private readonly _clearInterval = overridenWindowObject.clearInterval
    ...
    readonly clearTimeout = (id: number | undefined): void => {
        ...
        if (index > NOT_FOUND) tasks.splice(index, 1)
    }
