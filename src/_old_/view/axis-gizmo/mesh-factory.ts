...
function makeTexturePos(color: string, label?: string): CanvasTexture {
    ...
    ctx.strokeStyle = "#000"
    ctx.lineWidth = 2
    const x = half(TEXTURE_SIZE)
    const y = half(TEXTURE_SIZE)
    const radius = Math.floor(half(TEXTURE_SIZE)) - 1
    ctx.beginPath()
    ctx.ellipse(x, y, radius, radius, 0, 0, double(Math.PI))
    ctx.stroke()
    ctx.fill()
    if (label) {
        ...
        ctx.fillText(
            label,
            half(TEXTURE_SIZE - metrics.width),
            half(TEXT_ASJUST * TEXTURE_SIZE)
        )
    }
...
function makeTextureNeg(color: string): CanvasTexture {
    ...
    const x = half(SIZE)
    const y = half(SIZE)
    const radius = Math.floor(half(TIP_RADIUS * SIZE)) - 1
    ctx.beginPath()
    ctx.lineWidth = 8
    ctx.ellipse(x, y, radius, radius, 0, 0, double(Math.PI))
    ctx.strokeStyle = "#000"
