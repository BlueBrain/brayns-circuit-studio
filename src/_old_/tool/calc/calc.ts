...
import { half, HALF } from "@/_old_/constants"
 ...
 */
export default class Calc extends CalcInterface {
    /**
    ...
    computeBoundingBoxCenter(boundingBox: {
        min: Vector3
        max: Vector3
    }): Vector3 {
    ...
    multiplyQuaternions(...quaternions: Vector4[]): Vector4 {
        ...
        if (quaternions.length > MINIMUM_NB_QUAT_FOR_MULTIPLICATION) {
            return this.multiplyQuaternions(
                this.multiplyQuaternions(first, second),
                ...rest
            )
        }
    ...
    getEulerAnglesFromQuaternion(quaternion: Vector4): EulerAngles {
        ...
        // const eulerAngles: EulerAngles = { roll: 0, pitch: 0, yaw: 0 }
        // const THRESHOLD = 0.499999
        // const test = x * y + z * w
        // if (test > THRESHOLD) {
        //     // singularity at north pole
        //     return {
        //         yaw: double(Math.atan2(x, w)),
        //         pitch: half(Math.PI),
        //         roll: 0,
        //     }
        // }
        ...
        // const sinrCosp = double(w * x + y * z)
        // const cosrCosp = 1 - double(x * x + y * y)
        // eulerAngles.roll = Math.atan2(sinrCosp, cosrCosp)
        // // Pitch (y-axis rotation)
        // const sinp = double(w * y - z * x)
        // if (Math.abs(sinp) >= 1) {
        //     // Use 90 degrees if out of range
        //     eulerAngles.pitch = sinp > 0 ? Math.PI : -Math.PI
        // } else eulerAngles.pitch = Math.asin(sinp)
        // // yaw (z-axis rotation)
        // const sinyCosp = double(w * z + x * y)
        // const cosyCosp = 1 - double(y * y + z * z)
        // eulerAngles.yaw = Math.atan2(sinyCosp, cosyCosp)
        // return eulerAngles
    }
    ...
    normalizeVector<T extends Vector>(vector: T): T {
        const squareLength = vector.reduce(
            (sum, value) => sum + value * value,
            0
        )
        if (squareLength < Calc.EPSILON) vector
