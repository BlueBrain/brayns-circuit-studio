...
export function isSerializableData(data: unknown): data is SerializableData {
    if (["null", "boolean", "number", "string"].includes(typeof data)) {
        return true
    }
    if (Array.isArray(data)) {
        for (const item of data) {
            if (!isSerializableData(item)) return false
        }
    } else if (isObject(data)) {
        for (const key of Object.keys(data)) {
            const value = data[key]
            if (!isSerializableData(value)) return false
        }
    }
    return true
}
